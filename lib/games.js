...
exports.getScores = function getScores(input, cb) {
	...
	if (errorMessages.length > 0) {
		...
		db.Score.findAll({ where: { GameId: input.gameid }, order: 'value DESC', include: [db.Game] }).success(function(scores) {
			...
			for (var i = 0; i < total_count; i++) {
				_scores.push({ name: scores[i].name, value: scores[i].value, time: scores[i].createdAt });
			}
